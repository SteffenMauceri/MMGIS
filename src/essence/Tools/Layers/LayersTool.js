...
*/
    ...
    function traverseHeaderLayersResetChildrenon(node, parent) {
        for (var i = 0; i < node.length; i++) {
            if (node[i].type == 'header') {
                if (node[i].expanded) {
                    // Set all of the sub layer headers to be false
                    $(`#layersToolList > li#header_${node[i].name}`).attr('childrenon', false)
                }
            }

            if (node[i].sublayers)
                traverseHeaderLayersResetChildrenon(node[i].sublayers, node[i])
        }
    }
